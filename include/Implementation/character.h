...
class Character : public ichi::core::Component {
    public: 
        Character(
            ...
            ichi::graphics::AnimatedSprite& animation,
            bool grounded,
            ichi::Vector2D velocity = ichi::Vector2D()
        ): Component(hitbox), hp(hp), ms(ms), lives(lives), animation(animation), grounded(grounded), velocity(velocity) {}
