...
namespace ichi::core::textureManager { 
    ...
    bool addTextureFor(const Sprite& s, const std::string& filePath) {
        ...
        SDL_Texture* texture = SDL_CreateTextureFromSurface(Engine::getInstance()->getRenderer(), surf);
        if(texture == nullptr){
    ...
    void draw(const Sprite& s, int x, int y) {
        ...
        success = SDL_RenderCopy(Engine::getInstance()->getRenderer(), textureMap[s], &srcRect, &dstRect);
        if(success == 0) {
